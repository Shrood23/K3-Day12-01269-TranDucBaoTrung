...
class Lifecycle:
    ...
    def request_shutdown(self, signum=None, frame=None) -> None:
        ...
        """
        # Chỉ bật cờ: /health trả 503 → load balancer rút instance khỏi vòng xoay
        self.shutting_down = True

        # Nhường lại cho handler cũ (của uvicorn) — thứ thật sự dừng server.
        # Quên bước này thì app bật cờ rồi chạy tiếp cho tới khi bị SIGKILL.
        previous = self._previous.get(signum)
        if callable(previous):
            previous(signum, frame)
